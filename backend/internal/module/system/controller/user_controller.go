...
// UpdateProfile PUT /system/users/profile（登录即可）：修改本人基本资料（可选签名图 file_key、头像 URL），返回最新用户信息。
func (ctl *UserController) UpdateProfile(c *gin.Context) {
	...
	if be := ctl.svc.UpdateProfile(uid, req.Name, req.Phone, req.SignatureFileKey, req.Avatar); be != nil {
		response.Fail(c, be)
		return
	}
