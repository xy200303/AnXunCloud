...
// reported 待受理（项目关闭受理时上报直接进 pending_dispatch）
// → pending_dispatch 待派单（受理通过；受理驳回 → closed_invalid 已作废）
// → processing 处理中（派单指定维修工或抢单，即视为接单）
// → pending_confirm 待验收（维修工完工提交，附完工照片）
// → closed 已闭环（验收通过；验收不通过退回 processing 并记录退回原因）
const (
	OrderReported        = "reported"         // 待受理
	...
	OrderClosedInvalid   = "closed_invalid"   // 已作废（受理驳回）
)

// 工单来源（字典 order_source）
const (
	SourceInspection = "inspection" // 巡检异常转单（视同已受理，直接待派单）
...
type WorkOrder struct {
	...
	Category     string           `gorm:"size:32" json:"category"` // 工单分类（受理时填写）
	...
	// RejectReason 最近一次驳回原因（受理驳回 / 验收退回）
