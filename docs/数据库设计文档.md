...
CREATE TABLE sys_config (
    ...
    config_group varchar(50) NOT NULL DEFAULT 'system',      -- 分组（inspection/mp/msg/security/auth/system，前端按组做 Tab）
    remark      varchar(255) NOT NULL DEFAULT '',
    created_at  timestamptz  NOT NULL DEFAULT now(),
    updated_at  timestamptz  NOT NULL DEFAULT now(),
    deleted_at  timestamptz  NULL
...
CREATE TABLE inspection_point (
    ...
    credential           varchar(16)  NOT NULL DEFAULT 'qrcode',       -- 点位凭证：qrcode 扫码 / nfc NFC 卡 / none 免凭证
    ...
    updated_at           timestamptz  NOT NULL DEFAULT now(),
    deleted_at           timestamptz  NULL,
    CONSTRAINT chk_point_credential CHECK (credential IN ('qrcode','nfc','none')),
...
COMMENT ON COLUMN inspection_point.credential     IS '点位凭证：qrcode 扫码 / nfc NFC 卡 / none 免凭证';
...
| inspection_point.credential | qrcode / nfc / none | 扫码 / NFC / 免凭证（与 require_fence 组合，check 约束，不走字典） | — |
