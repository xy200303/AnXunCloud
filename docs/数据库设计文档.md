...
- **二维码编号 qrcode_no**：`P` + 6 位序列（如 `P000101`，序号源为 PG 序列 `qrcode_no_seq`，与 UUID 主键解耦，便于打印张贴与口头报号）；二维码内容即编号本身（最短内容、alphanumeric 编码集，识别率最高），小程序扫码后直接使用，兼容解析早期 `inspection://checkin?no=` 前缀格式。
