...
CREATE TABLE sys_config (
    ...
    config_group varchar(50) NOT NULL DEFAULT 'system',      -- 分组（inspection/mp/msg/security/auth/map/system，前端按组做 Tab）
    remark      varchar(255) NOT NULL DEFAULT '',
    created_at  timestamptz  NOT NULL DEFAULT now(),
    updated_at  timestamptz  NOT NULL DEFAULT now(),
    deleted_at  timestamptz  NULL
...
CREATE TABLE inspection_point (
    ...
    credential           varchar(16)  NOT NULL DEFAULT 'qrcode',       -- 点位凭证：qrcode 扫码 / nfc NFC 卡 / any 任一（扫码或 NFC）/ none 免凭证
    require_fence        boolean      NOT NULL DEFAULT false,          -- 是否要求 GPS 在围栏内（与 credential 独立，可叠加）
    ...
    updated_at           timestamptz  NOT NULL DEFAULT now(),
    deleted_at           timestamptz  NULL,
    CONSTRAINT chk_point_credential CHECK (credential IN ('qrcode','nfc','none','any')),
    ...
    CONSTRAINT chk_point_check_valid CHECK (credential <> 'none' OR require_fence)  -- 凭证与围栏至少启用一项
);
...
COMMENT ON COLUMN inspection_point.credential     IS '点位凭证：qrcode 扫码 / nfc NFC 卡 / any 任一（扫码或 NFC）/ none 免凭证';
COMMENT ON COLUMN inspection_point.require_fence  IS '是否要求 GPS 在围栏内（与 credential 独立，同时启用都须通过）';
...
| inspection_point.credential | qrcode / nfc / any / none | 扫码 / NFC / 任一（扫码或 NFC）/ 免凭证（与 require_fence 组合，check 约束，不走字典） | — |
...
- **二维码编号 qrcode_no**：`P` + 6 位序列（如 `P000101`，序号源为 PG 序列 `qrcode_no_seq`，与 UUID 主键解耦，便于打印张贴与口头报号）；二维码内容为短链接 `{APP_BASE_URL}/p/{qrcode_no}`（外来人员用微信/相机扫码直接打开点位信息公开页），巡检端扫码后提取编号使用，兼容解析旧版纯编号贴纸与早期 `inspection://checkin?no=` 前缀格式。
...
按 §4.1 总表全量预置：`common_status`、`user_type`、`data_scope`、`menu_type`、`building_type`、`point_type`、`cycle_type`、`task_status`、`checkin_type`、`checkin_result`、`order_priority`、`work_order_status`。取值与 label 对照即 §4.1「含义」列，此处不再重复罗列。
...
| map.tencent_key | 腾讯地图Key | （空） | 申请地址 https://lbs.qq.com/ ，需绑定调用域名白名单；用于 PC 后台点位地图选点 |
...
|---|---|---|---|---|
| `session:admin:{userId}:{accessJti}` | Hash | `{tokenId, refreshId, name, roles, loginAt}` | 7d（=refresh 有效期） | 后台登录会话；按登录点独立存储支持多点登录，JWT 校验通过后按 jti 精确匹配会话，登出仅删当前登录点，改密/停用删全部 |
| `session:mp:{userId}:{accessJti}` | Hash | `{tokenId, refreshId, name, roles, loginAt}` | 7d | 小程序/App 登录会话（微信用户无感登录，有效期长）；按登录点独立，同后台 |
