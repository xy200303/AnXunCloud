...
export function createNotice(data: { title: string; content: string; status?: number; attachments?: NoticeAttachment[] }) {
  return request<{ id: string }>({ url: '/system/notices', method: 'post', data })
}

export function updateNotice(id: string, data: { title?: string; content?: string; status?: number; attachments?: NoticeAttachment[] }) {
